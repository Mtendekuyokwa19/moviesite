...
export function Card({ removeFromCart, movie, AddtoCart, toogleWatchlist, movies }: ICard) {
  ...
  return (
    <div style={{
      ...
      <div className="flex flex-col justify-center items-center h-screen">
        ...
        <MovieCard removeFromCart={removeFromCart} movie={movie} AddtoCart={AddtoCart} toogleWatchlist={toogleWatchlist} Movies={movies} />
      </div>
    </div>
  )
...
function MovieCard({ movie, AddtoCart, toogleWatchlist, Movies, removeFromCart }: ImovieCard) {



  return (
    <div className="sm:w-1/2  sm:h-4/5 md:w-1/2 md:flex md:flex-col md:h-3/6 md:rounded-lg  h-4/5 rounded-md" id="modal">
      <div>
        <Link to={"/Shop"}>
      ...
      <div className="flex bg-white z-10 sm:h-3/5 md:h-3/5  justify-evenly flex-1  p-5 rounded-sm">
        <MovieResearch removeFromCart={removeFromCart} movie={movie} AddtoCart={AddtoCart} toogleWatchlist={toogleWatchlist} Movies={Movies} />
...
function MovieResearch({ movie, AddtoCart, toogleWatchlist, removeFromCart, Movies }: ImovieCard) {
  ...
  return (
    <div className="flex flex-col gap-5 p-5">
      ...
      <MovieButton AddtoCart={AddtoCart} movie={movie} toogleWatchlist={toogleWatchlist} removeFromCart={removeFromCart} Movies={Movies} />
    </div>
  )
}
...
function MovieButton({ movie, removeFromCart, AddtoCart, toogleWatchlist, Movies }: ImovieCard) {
